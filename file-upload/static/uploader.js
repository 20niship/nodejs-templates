...
class HugeUploader {
    ...
    /**
     * Called on net failure. If retry counter !== 0, retryfileRetry after delayBeforeRetry
     */
    _manageRetries() {
        if (this.retriesCount++ < this.retries) {
            ...
            this.events["fileRetry"]({ detail: { message: `An error occured uploading chunk ${this.chunkCount}. ${this.retries - this.retriesCount} retries left`, chunk: this.chunkCount, retriesLeft: this.retries - this.retriesCount }})
            return;
        }
        ...
        this.events["error"]( { detail: `An error occured uploading chunk ${this.chunkCount}. No more retries, stopping upload` })
    }
    ...
    _sendChunks() {
        ...
        .then((res) => {
            if (res.status === 200 || res.status === 201 || res.status === 204) {
                ...
                this.events["progress"]({ detail: percentProgress })
            }
